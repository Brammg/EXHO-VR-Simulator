	...
	SubShader
	{
		Pass
		{
			...
			Varyings Vert(Attributes v)
			{
				...
				{
					half sss = 0.0;
					const float3 uv_slice_smallerLod = WorldToUV(positionWS_XZ_before, oceanPosScale0, oceanParams0, sliceIndex0);
					SampleDisplacements(_LD_TexArray_AnimatedWaves, uv_slice_smallerLod, wt_smallerLod, worldPos, sss);
				}
				if (wt_biggerLod > 0.001)
				{
					half sss = 0.0;
					const float3 uv_slice_biggerLod = WorldToUV(positionWS_XZ_before, oceanPosScale1, oceanParams1, sliceIndex0 + 1.0);
