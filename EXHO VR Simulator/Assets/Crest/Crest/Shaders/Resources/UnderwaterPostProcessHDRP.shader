		...
		Pass
		{
			...
			half3 ApplyUnderwaterEffect(
				...
				float3 displacement = 0.0;
				{
					...
					float sliceIndex = clamp(_DataSliceOffset, 0.0, _SliceCount - 2.0);
					int sliceIndexI = (int)sliceIndex;
					float3 uv_slice = WorldToUV(_WorldSpaceCameraPos.xz, _LD_Pos_Scale[sliceIndexI], _LD_Params[sliceIndexI], sliceIndex);
