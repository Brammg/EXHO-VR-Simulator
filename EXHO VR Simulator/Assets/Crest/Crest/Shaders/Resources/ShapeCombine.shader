	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 Frag(Varyings input) : SV_Target
			{
				...
				float nextSliceIndex = _LD_SliceIndex + 1.0;
				const float3 uv_nextLod = WorldToUV(worldPosXZ, _LD_Pos_Scale[nextSliceIndex], _LD_Params[nextSliceIndex], nextSliceIndex);
				...
				float3 uv_thisLod_flow_0 = WorldToUV(worldPosXZ - offsets[0] * flow, _LD_Pos_Scale[_LD_SliceIndex], _LD_Params[_LD_SliceIndex], _LD_SliceIndex);
				float3 uv_thisLod_flow_1 = WorldToUV(worldPosXZ - offsets[1] * flow, _LD_Pos_Scale[_LD_SliceIndex], _LD_Params[_LD_SliceIndex], _LD_SliceIndex);
